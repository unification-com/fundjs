...
export interface UseAllStreamsByPairQuery<TData> extends ReactQueryParams<QueryAllStreamsByPairResponse, TData> {
  request: QueryAllStreamsByPairRequest;
}
...
export const createRpcQueryHooks = (rpc: ProtobufRpcClient | undefined) => {
  ...
  const useAllStreamsByPair = <TData = QueryAllStreamsByPairResponse,>({
    request,
    options
  }: UseAllStreamsByPairQuery<TData>) => {
    return useQuery<QueryAllStreamsByPairResponse, Error, TData>(["allStreamsByPairQuery", request], () => {
      if (!queryService) throw new Error("Query Service not initialized");
      return queryService.allStreamsByPair(request);
    }, options);
  };
